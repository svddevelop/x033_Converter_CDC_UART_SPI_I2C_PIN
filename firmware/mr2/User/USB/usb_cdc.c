...
void CDC_writeLine(const char *str) {
    CDC_writeString(str);
    CDC_write('\r');
    CDC_write('\n');
}
...
void CDC_writeHex(uint32_t value, uint8_t digits) {
    ...
    CDC_writeBuffer(buffer, digits);
}

// Write decimal number
void CDC_writeDec(int32_t value) {
    char buffer[12];
    int length = 0;
...
// Write formatted string to OUT buffer (printf-like)
void CDC_printf(const char *format, ...) {
    char buffer[128]; // §­§à§Ü§Ñ§Ý§î§ß§í§Û §Ò§å§æ§Ö§â §Õ§Ý§ñ §æ§à§â§Þ§Ñ§ä§Ú§â§à§Ó§Ñ§ß§Ú§ñ
    va_list args;
    
    va_start(args, format);
    int length = _vfprintf(buffer, sizeof(buffer), format, args);
    va_end(args);
    
    if (length > 0) {
        CDC_writeBuffer(buffer, (uint16_t)length);
    }
}

// §¢§Ö§Ù§à§á§Ñ§ã§ß§Ñ§ñ §Ó§Ö§â§ã§Ú§ñ §ã §à§Ô§â§Ñ§ß§Ú§é§Ö§ß§Ú§Ö§Þ §Õ§Ý§Ú§ß§í
void CDC_printf_safe(const char *format, ...) {

    char buffer[256]; // §µ§Ó§Ö§Ý§Ú§é§Ö§ß§ß§í§Û §Ò§å§æ§Ö§â
    va_list args;
    ...
    if (length > 0) {
        // §°§Ô§â§Ñ§ß§Ú§é§Ú§Ó§Ñ§Ö§Þ §Õ§Ý§Ú§ß§å §â§Ñ§Ù§Þ§Ö§â§à§Þ §Ò§å§æ§Ö§â§Ñ
